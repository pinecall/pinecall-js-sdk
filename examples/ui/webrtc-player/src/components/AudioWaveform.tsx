...
export default function AudioWaveform({ userMetricsRef, botMetricsRef, isActive }: Props) {
  ...
  const labelText = activeSrc === 'user' ? 'You' : activeSrc === 'bot' ? 'Agent' : 'Idle';

  return (
    <div className="w-full" style={{ background: 'rgba(18, 9, 28, 0.5)', borderRadius: '12px', padding: '12px 16px' }}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-[11px] font-medium uppercase tracking-wider" style={{ color: 'var(--pc-text-medium)', opacity: 0.6 }}>Audio</span>
        <span className="text-[10px] font-medium tracking-wide transition-colors duration-300" style={{ color: labelColor }}>{labelText}</span>
      </div>
      <canvas
        ref={canvasRef}
        className="w-full rounded-md"
        style={{ height: isActive ? '40px' : '20px', transition: 'height 0.3s ease' }}
      />
    </div>
  );
}
